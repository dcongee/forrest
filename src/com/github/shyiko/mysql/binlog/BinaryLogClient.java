...
public class BinaryLogClient implements BinaryLogClientMXBean {
	private static Logger logger = Logger.getLogger(BinaryLogClient.class);
	...
	public BinaryLogClient(String hostname, int port, int serverid, String username, String password,
			...
			try {
				...
				if (cancelDisconnect != null) {
					try {
						...
						logger.warn("\"" + e.getMessage() + "\" was thrown while canceling scheduled disconnect call");
					}
				}
			}
			connected = true;
		...
		final Thread thread = newNamedThread(new Runnable() {
			...
			public void run() {
				...
				if (connectLatch.getCount() != 0) {
					logger.warn("Failed to establish connection in " + timeout + "ms. " + "Forcing disconnect.");
					try {
	...
	private void requestBinaryLogStream() throws IOException {
		...
		if (b[0] == -1) {
			...
			String errorSqlState = ForrestDataUtil.byteArrayToStr(sqlState);
			int errorSqlCode = ForrestDataUtil.byteArrayToInt(errorCode);
			// System.out.print(ForrestDataUtil.byteArrayToInt(errorCode) + ": ");
			// logger.error(
			// ForrestDataUtil.byteArrayToStr(sqlState) + " " +
			// ForrestDataUtil.byteArrayToInt(errorCode) + ": ");
			...
			// System.out.println(this.getBinlogFilename() + " " +
			// this.getBinlogPosition());
			String errorSqlMessage = ForrestDataUtil.byteArrayToStr(errorMessage);
		...
		try {
			while (inputStream.peek() != -1) {
				...
				if (marker == 0xFF) { // 0xFF = 255
					...
					error.append("ERROR ").append(errorPacket.getErrorCode()).append(" (");
					error.append(errorPacket.getSqlState()).append("): ").append(errorPacket.getErrorMessage());
		...
		synchronized (eventListeners) {
			for (EventListener eventListener : eventListeners) {
				try {
					...
					logger.warn(eventListener + " choked on " + event + e.getMessage());
				}
			}
		}
	}
	...
	public interface LifecycleListener {
		...
		/**
		 * It's guarantied to be called before {@link #onDisconnect(BinaryLogClient)})
