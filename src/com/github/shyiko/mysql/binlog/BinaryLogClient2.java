 ...
 */
public class BinaryLogClient2 implements BinaryLogClientMXBean {
	 ...
	 * username, password).
	 * 
	 * @see BinaryLogClient2#BinaryLogClient(String, int, String, String, String)
	 */
	public BinaryLogClient2(String username, String password) {
	 ...
	 * @see BinaryLogClient2#BinaryLogClient(String, int, String, String, String)
	 */
	public BinaryLogClient2(String schema, String username, String password) {
		this("localhost", 3306, schema, username, password);
	}

	/**
	 * Alias for BinaryLogClient(hostname, port, &lt;no schema&gt; = null, username,
	 * password).
	 * 
	 * @see BinaryLogClient2#BinaryLogClient(String, int, String, String, String)
	 */
	public BinaryLogClient2(String hostname, int port, String username, String password) {
		this(hostname, port, null, username, password);
	}

	public BinaryLogClient2(String hostname, int port, int serverid, String username, String password,
			String binlogFilename, long binlogPosition, String schema) {
	 ...
	 *            mysql server port
	 * @param schema
	 *            database name, nullable. Note that this parameter has nothing to
	 *            do with event filtering. It's used only during the authentication.
	 * @param username
	 *            login name
	 * @param password
	 *            password
	 */
	public BinaryLogClient2(String hostname, int port, String schema, String username, String password) {
		this.hostname = hostname;
		this.port = port;
		this.schema = schema;
		this.username = username;
	...
	/**
	 * @param blocking
	 *            blocking mode. If set to false - BinaryLogClient will disconnect
	...
	/**
	 * @param serverId
	 *            server id (in the range from 1 to 2^32 - 1). This value MUST be
	 *            unique across whole replication group (that is, different from any
	 *            other server id being used by any master or slave). Keep in mind
	 *            that each binary log client
	 *            (mysql-binlog-connector-java/BinaryLogClient, mysqlbinlog, etc)
	 *            should be treated as a simplified slave and thus MUST also use a
	 *            different server id.
	 * @see #getServerId()
	 */
	...
	/**
	 * @return binary log filename, nullable (and null be default). Note that this
	 *         value is automatically tracked by the client and thus is subject to
	 *         change (in response to {@link EventType#ROTATE}, for example).
	 ...
	 *            binary log filename. Special values are:
	 *            <ul>
	 *            <li>null, which turns on automatic resolution (resulting in the
	 *            last known binlog and position). This is what happens by default
	 *            when you don't specify binary log filename explicitly.</li>
	 ...
	 *         position of first event). Note that this value changes with each
	 *         incoming event.
	 ...
	 *            binary log position. Any value less than 4 gets automatically
	 *            adjusted to 4 on connect.
	...
	/**
	 * @return GTID set. Note that this value changes with each received GTID event
	 *         (provided client is in GTID mode).
	 ...
	 *            <p>
	 *            NOTE #1: Any value but null will switch BinaryLogClient into a
	 *            GTID mode (this will also set binlogFilename to "" (provided it's
	 *            null) forcing MySQL to send events starting from the oldest known
	 *            binlog (keep in mind that connection will fail if gtid_purged is
	 *            anything but empty (unless
	 *            {@link #setGtidSetFallbackToPurged(boolean)} is set to true))).
	 *            <p>
	 *            NOTE #2: {@link #setBinlogFilename(String)} and
	 *            {@link #setBinlogPosition(long)} can be used to specify the exact
	 *            position from which MySQL server should start streaming events
	 *            (taking into account GTID set).
	 *            <p>
	 *            NOTE #3: GTID set is automatically updated with each incoming GTID
	...
	/**
	 * @param gtidSetFallbackToPurged
	 *            true if gtid_purged should be used as a fallback when gtidSet is
	 *            set to "" and MySQL server has purged some of the binary logs,
	...
	/**
	 * @param keepAlive
	 *            true if "keep alive" thread should be automatically started
	 *            (recommended and true by default), false otherwise.
	 * @see #isKeepAlive()
	 ...
	 *            <li>if {@link #setKeepAlive(boolean)} is on then keepAlive thread
	 *            will attempt to reconnect if no HEARTBEAT events were received
	 *            within {@link #setKeepAliveInterval(long)} (instead of trying to
	 *            send PING every {@link #setKeepAliveInterval(long)}, which is
	 *            fundamentally flawed -
	 *            https://github.com/shyiko/mysql-binlog-connector-java/issues/118).
	 *            </ul>
	 *            Note that when used together with keepAlive heartbeatInterval MUST
	 *            be set less than keepAliveInterval.
	 *
	...
	/**
	 * @param socketFactory
	 *            custom socket factory. If not provided, socket will be created
	 *            with "new Socket()".
	 */
	...
	/**
	 * @param threadFactory
	 *            custom thread factory. If not provided, threads will be created
	 *            using simple "new Thread()".
	 */
			...
			try {
				...
				if (cancelDisconnect != null) {
					try {
						...
						if (logger.isLoggable(Level.WARNING)) {
							logger.warning(
									"\"" + e.getMessage() + "\" was thrown while canceling scheduled disconnect call");
						}
					}
				}
			}
	...
	private Callable scheduleDisconnectIn(final long timeout) {
		final BinaryLogClient2 self = this;
	...
	private void requestBinaryLogStream() throws IOException {
		...
		if (b[0] == -1) {
			...
			// System.out.println(Test1.hexStr2Str(Test1.bytesToHexFun2(new byte[] { b[0]
			// })));
			// System.out.println(Test1.bytesToHexFun2(new byte[] { b[0] }));
			// System.out.println(Test1.byteArrayToStr(b));
			...
			System.out.print(ForrestDataUtil.byteArrayToStr(sqlState) + " ");
			System.out.print(ForrestDataUtil.byteArrayToInt(errorCode) + ": ");
			byte[] errorMessage = new byte[b.length - 9];
			System.arraycopy(b, 9, errorMessage, 0, errorMessage.length);
			System.out.println(ForrestDataUtil.byteArrayToStr(errorMessage));
			System.out.println(this.getBinlogFilename() + " " + this.getBinlogPosition());
			System.exit(1);
		}
		// ErrorPacket error = new ErrorPacket(b);
		// System.out.println("error_code=" + error.getErrorCode());
		// System.out.println(error.getSqlState());
		// System.out.println(error.getErrorMessage());

		// String errorCode = new String(b);
	...
	public void connect(final long timeout) throws IOException, TimeoutException {
		...
		AbstractLifecycleListener connectListener = new AbstractLifecycleListener() {
			@Override
			public void onConnect(BinaryLogClient2 client) {
	...
	private void listenForEventPackets() throws IOException {
		...
		try {
			while (inputStream.peek() != -1) {
				...
				// int packetNumber = inputStream.readInteger(1);
				// int affectedRow = inputStream.readInteger(1);
				// int lastInsertID = inputStream.readInteger(1);
				// System.out.println("PacketLength: " + packetLength + " packetNumber: " +
				// packetNumber + " affectedRow: "
				// + affectedRow + " lastInsertID: " + lastInsertID);
				// System.out.println("PacketLength: " + packetLength);

				inputStream.skip(1); // 1 byte for sequence
				int marker = inputStream.read();
				if (marker == 0xFF) { // 0xFF = 255
					System.out.println("marker===" + marker);
					ErrorPacket errorPacket = new ErrorPacket(inputStream.read(packetLength - 1));
					System.out.println(errorPacket.getErrorCode());
					System.out.println(errorPacket.getSqlState());
	...
	private void updateClientBinlogFilenameAndPosition(Event event) {
		...
		// do not update binlogPosition on TABLE_MAP so that in case of reconnect (using
		// a different instance of
		// client) table mapping cache could be reconstructed before hitting row
		// mutation event
	...
	/**
	 * Register event listener. Note that multiple event listeners will be called in
	...
	/**
	 * Register lifecycle listener. Note that multiple lifecycle listeners will be
	 * called in order they where registered.
	 */
	 ...
	 * binlogFilename/binlogPosition to be cleared out. As the result following
	 * {@link #connect()} resumes client from where it left off.
	 */
	...
	/**
	 * {@link BinaryLogClient2}'s event listener.
	 */
	...
	/**
	 * {@link BinaryLogClient2}'s lifecycle listener.
	 */
	public interface LifecycleListener {

		/**
		 * Called once client has successfully logged in but before started to receive
		 * binlog events.
		 */
		void onConnect(BinaryLogClient2 client);

		/**
		 * It's guarantied to be called before {@link #onDisconnect(BinaryLogClient2)})
		 * in case of communication failure.
		 */
		void onCommunicationFailure(BinaryLogClient2 client, Exception ex);

		/**
		 * Called in case of failed event deserialization. Note this type of error does
		 * NOT cause client to disconnect. If you wish to stop receiving events you'll
		 * need to fire client.disconnect() manually.
		 */
		void onEventDeserializationFailure(BinaryLogClient2 client, Exception ex);

		/**
		 * Called upon disconnect (regardless of the reason).
		 */
		void onDisconnect(BinaryLogClient2 client);
	}
	...
	public static abstract class AbstractLifecycleListener implements LifecycleListener {
		...
		@Override
		public void onCommunicationFailure(BinaryLogClient2 client, Exception ex) {
		}

		@Override
		public void onEventDeserializationFailure(BinaryLogClient2 client, Exception ex) {
		}

		@Override
		public void onDisconnect(BinaryLogClient2 client) {
		}
