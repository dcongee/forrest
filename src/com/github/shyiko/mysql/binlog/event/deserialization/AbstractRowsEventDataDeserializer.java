...
/**
 * Whole class is basically a mix of
 * <a href="https://code.google.com/p/open-replicator">open-replicator</a>'s
 * AbstractRowEventParser and MySQLUtils. Main purpose here is to ease rows
 * deserialization.
 * <p>
 *
...
public abstract class AbstractRowsEventDataDeserializer<T extends EventData> implements EventDataDeserializer<T> {
	...
	protected Serializable deserializeDate(ByteArrayInputStream inputStream) throws IOException {
		...
		if (year == 0 || month == 0 || day == 0) {
			return invalidDate(year, month, day);
		}
	...
	private Serializable invalidDate(int year, int month, int day) {
		String syear = (year == 0 ? "0000" : formatYear(year));
		String smonth = (month == 0 ? "00" : formatMonthAndDay(month));
		String sday = (day == 0 ? "00" : formatMonthAndDay(day));
		return syear + "-" + smonth + "-" + sday;
	}

	private String formatMonthAndDay(int dateNum) {
	...
	protected Serializable deserializeTimeV2(int meta, ByteArrayInputStream inputStream) throws IOException {
		 ...
		 * (in big endian)
		 * 
		 * 1 bit sign (1= non-negative, 0= negative) 1 bit unused (reserved for future
		 * extensions) 10 bits hour (0-838) 6 bits minute (0-59) 6 bits second (0-59)
		 * 
		 * (3 bytes in total)
		 * 
		...
		Long timestamp = asUnixTime(1970, 1, 1, bitSlice(time, 2, 10, 24), bitSlice(time, 12, 6, 24),
				bitSlice(time, 18, 6, 24), fsp / 1000);
		if (deserializeDateAndTimeAsLong) {
	...
	protected Serializable deserializeTimestampV2(int meta, ByteArrayInputStream inputStream) throws IOException {
		...
		// return new java.sql.Timestamp(timestamp); //原来的实现无法对微秒进行转换

		// 没找到java.sql.Timestamp对微秒进行转换的方法，改成下面的实现
		millis = millis * 1000;
		// System.out.println("millis " + millis + " meta " + meta + " fsp " + fsp + " "
		// + timestamp + " timestamp");
		switch (meta) {
		case 0:
			return millis != 0 ? new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new java.sql.Timestamp(millis))
					: "0000-00-00 00:00:00";

		case 1:
			return millis != 0
					? new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.").format(new java.sql.Timestamp(millis))
							+ String.format("%01d", fsp / 100000)
					: "0000-00-00 00:00:00.0";

		case 2:
			return millis != 0
					? new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.").format(new java.sql.Timestamp(millis))
							+ String.format("%02d", fsp / 10000)
					: "0000-00-00 00:00:00.00";

		case 3:
			return millis != 0
					? new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.").format(new java.sql.Timestamp(millis))
							+ String.format("%03d", fsp / 1000)
					: "0000-00-00 00:00:00.000";

		case 4:
			return millis != 0
					? new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.").format(new java.sql.Timestamp(millis))
							+ String.format("%04d", fsp / 100)
					: "0000-00-00 00:00:00.0000";

		case 5:
			return millis != 0
					? new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.").format(new java.sql.Timestamp(millis))
							+ String.format("%05d", fsp / 10)
					: "0000-00-00 00:00:00.00000";

		default:
			return millis != 0
					? new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.").format(new java.sql.Timestamp(millis))
							+ String.format("%06d", fsp) // fsp不足6位前补0，因为当微秒为000001，fsp为1.
					: "0000-00-00 00:00:00.000000";
		}
	}

	protected Serializable createTimestamp(int meta, long timestamp) {

		return castTimestamp(null, 0);
	}
	...
	protected Serializable deserializeDatetimeV2(int meta, ByteArrayInputStream inputStream) throws IOException {
		 ...
		 * (in big endian)
		 * 
		 * 1 bit sign (1= non-negative, 0= negative) 17 bits year*13+month (year 0-9999,
		...
		int fsp = deserializeFractionalSeconds(meta, inputStream);
		Long timestamp = asUnixTime(yearMonth / 13, yearMonth % 13, bitSlice(datetime, 18, 5, 40),
				bitSlice(datetime, 23, 5, 40), bitSlice(datetime, 28, 6, 40), bitSlice(datetime, 34, 6, 40),
				fsp / 1000);
		if (deserializeDateAndTimeAsLong) {
			return castTimestamp(timestamp, fsp);
		}
		// return timestamp != null ? new java.util.Date(timestamp) : null;   //转换出来的时间格式为JAVA的DATE类型，与数据库不一致。

	     
		//原来的实现，日期与mysql时间表示不一致。改成下面的实现
		switch (meta) {
		case 0:
			return timestamp != 0 ? new SimpleDateFormat("yyyy-MM-ddHH:mm:ss").format(new java.sql.Date(timestamp))
					: "0000-00-00 00:00:00";

		case 1:
			return timestamp != 0
					? new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.").format(new java.util.Date(timestamp))
							+ String.format("%01d", fsp / 100000)
					: "0000-00-00 00:00:00.0";
	...
	protected Serializable deserializeString(int length, ByteArrayInputStream inputStream) throws IOException {
		// charset is not present in the binary log (meaning there is no way to
		// distinguish between CHAR / BINARY)
	...
	/**
	 * Deserialize the {@code JSON} value on the input stream, and return MySQL's
	 * internal binary representation of the JSON value. See
	 * {@link com.github.shyiko.mysql.binlog.event.deserialization.json.JsonBinary}
	 * for a utility to parse this binary representation into something more useful,
	 * including a string representation.
	 *
	 * @param meta
	 *            the number of bytes in which the length of the JSON value is found
	 *            first on the input stream
	 * @param inputStream
	 *            the stream containing the JSON value
	 * @return the MySQL internal binary representation of the JSON value; may be
	 *         null
	 * @throws IOException
	...
	protected Long asUnixTime(int year, int month, int day, int hour, int minute, int second, int millis) {
		...
		return UnixTime.fromMe(year, month, day, hour, minute, second, millis);
	}
	...
	static class UnixTime {
		...
		private static final int[] YEAR_DAYS_BY_MONTH = new int[] { 0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304,
				334, 365 };
		private static final int[] LEAP_YEAR_DAYS_BY_MONTH = new int[] { 0, 31, 60, 91, 121, 152, 182, 213, 244, 274,
				305, 335, 366 };

		/**
		 * Calendar::getTimeInMillis but magnitude faster for all dates starting from
		 * October 15, 1582 (Gregorian Calendar cutover).
		 *
		 * @param year
		 *            year
		 * @param month
		 ...
		 *            [0..999]
		 *
		 * @return Unix time (number of seconds that have elapsed since 00:00:00 (UTC),
		 ...
		 * @return
		 */
		public static long fromMe(int year, int month, int day, int hour, int minute, int second, int millis) {
			return fallbackToGC(year, month, day, hour, minute, second, millis);
		}

		// checkstyle, please ignore ParameterNumber for the next line
		private static long fallbackToGC(int year, int month, int dayOfMonth, int hourOfDay, int minute, int second,
				int millis) {
			// Calendar c = Calendar.getInstance(TimeZone.getTimeZone("GMT"));
			Calendar c = Calendar.getInstance(TimeZone.getDefault()); // 获取系统默认时区
	...
	public static void main(String[] args) {
		// 1534517713666
		// Long t = 1534577749353l;
		// Date da = new Date(t);
		// System.out.println(da);
		// System.out.println(new java.sql.Timestamp(t));
		long time = System.currentTimeMillis();
		System.out.println(time);
		// SimpleDateFormat fmt = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSSSSS"); //
		// // 精确到毫秒
		// String suffix = fmt.format(new java.util.Date(time));
		// System.out.println(suffix);
		System.out.println(Math.round(382566 / 10));
		System.out.println(
				new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSSSSS").format(new java.util.Date(-30609711458000l)));

		System.out.println(new SimpleDateFormat("yyyy-MM-dd").format(new java.sql.Date(0, 0, 0)));

		System.out.println(String.format("%06d", 12000));
